...
class Actor(torch.nn.Module):
    ...
    def __init__(self, dim_observations: int =10, dim_actions: int= 2, nn_l1: int = 512, nn_l2: int = 512):
        ...
        self.final_layer_mu = torch.nn.Linear(nn_l2, dim_actions)
        self.final_log_sigma = torch.nn.Linear(nn_l2, dim_actions)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mu = torch.nn.functional.sigmoid(self.final_layer_mu(x))
        log_sigma = torch.nn.functional.sigmoid(self.final_log_sigma(x))
        return mu, log_sigma
...
class DDPGAgent:
    ...
    def act(self, state : torch.Tensor) -> torch.Tensor:
        ...
        action, _, _ = self.actor_params_to_action(mu, log_sigma)
        action = action.cpu().data.numpy() 
        action = np.clip(action, 0, 1)
        return torch.Tensor(action)
    
    def actor_params_to_action(self, mu, log_sigma):
